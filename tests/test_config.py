...
def test_from_json_valid_config() -> None:
...
def test_from_json_with_path_object() -> None:
...
def test_env_variable_substitution() -> None:
...
def test_env_variable_substitution_missing_var() -> None:
...
def test_env_variable_no_substitution_for_non_empty() -> None:
    """Test that non-empty env values are not replaced"""
    config_data = {
        "mcp_servers": {"test_server": {"command": "test_cmd", "args": [], "env": {"EXISTING_KEY": "existing_value"}}}
    }
...
def test_multiple_servers_with_env_vars() -> None:
...
def test_server_without_env() -> None:

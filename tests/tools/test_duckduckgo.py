...
# Extract content logic tests
"""Test the content extraction logic without decorators."""


def _extract_content_impl(url: str) -> str:
...
@patch("httpx.get")
def test_extract_content_success_with_main_tag(mock_get):
    ...
    result = _extract_content_impl("https://example.com")
...
@patch("httpx.get")
def test_extract_content_success_with_article_tag(mock_get):
...
@patch("httpx.get")
def test_extract_content_success_with_content_div(mock_get):
...
@patch("httpx.get")
def test_extract_content_fallback_to_full_text(mock_get):
...
@patch("httpx.get")
def test_extract_content_removes_unwanted_elements(mock_get):
...
@patch("httpx.get")
def test_extract_content_handles_empty_lines(mock_get):
...
@patch("httpx.get")
def test_extract_content_http_error(mock_get):
    """Test handling of HTTP errors."""
    mock_get.side_effect = httpx.HTTPStatusError("Not found", request=Mock(), response=Mock(status_code=404))

    result = _extract_content_impl("https://example.com/nonexistent")
...
@patch("httpx.get")
def test_extract_content_network_error(mock_get):
    """Test handling of network errors."""
    mock_get.side_effect = httpx.NetworkError("Connection failed")

    result = _extract_content_impl("https://unreachable.example.com")
...
@patch("httpx.get")
def test_extract_content_timeout_error(mock_get):
    """Test handling of timeout errors."""
    mock_get.side_effect = httpx.TimeoutException("Request timeout")

    result = _extract_content_impl("https://slow.example.com")
...
@patch("httpx.get")
def test_extract_content_parsing_error(mock_get):
...
@patch("httpx.get")
def test_extract_content_empty_response(mock_get):
...
def test_extract_content_uses_correct_timeout():
    ...
    with patch("httpx.get") as mock_get:
        ...
        _extract_content_impl("https://example.com")
...
# Web search logic tests
"""Test the web search logic without decorators."""


def _web_search_impl(queries: list[str], max_results_per_query: int = 2) -> str:
...
def test_web_search_single_query_success(mock_ddgs_class):
    ...
    result = _web_search_impl(["python tutorial"])
...
@patch("duckduckgo_search.DDGS")
def test_web_search_multiple_queries_success(mock_ddgs_class):
    ...
    result = _web_search_impl(["python", "django"])
...
@patch("duckduckgo_search.DDGS")
def test_web_search_custom_max_results(mock_ddgs_class):
    ...
    result = _web_search_impl(["test query"], max_results_per_query=3)
...
@patch("duckduckgo_search.DDGS")
def test_web_search_empty_queries_list(mock_ddgs_class):
...
@patch("duckduckgo_search.DDGS")
def test_web_search_no_results(mock_ddgs_class):
    """Test web search when no results are found."""
    mock_ddgs = Mock()
    mock_ddgs.text.return_value = []
    mock_ddgs_class.return_value = mock_ddgs

    result = _web_search_impl(["nonexistent query"])
...
@patch("duckduckgo_search.DDGS")
def test_web_search_ddgs_text_error(mock_ddgs_class):
...
@patch("duckduckgo_search.DDGS")
def test_web_search_result_formatting(mock_ddgs_class):
    ...
    result = _web_search_impl(["query1", "query2"])
...
@patch("duckduckgo_search.DDGS")
def test_web_search_special_characters_in_query(mock_ddgs_class):
    ...
    result = _web_search_impl([special_query])
...
@patch("duckduckgo_search.DDGS")
def test_web_search_unicode_query(mock_ddgs_class):
    ...
    result = _web_search_impl([unicode_query])
...
def test_function_tool_decorators_present():
...
def test_extract_content_robust_api_call_decorator(mock_get):
...
@patch("httpx.get")
@patch("duckduckgo_search.DDGS")
def test_search_and_extract_workflow(mock_ddgs_class, mock_get):
    ...
    search_result = _web_search_impl(["test article"])
    assert "https://example.com/article" in search_result

    # Then extract content from the URL
    content = _extract_content_impl("https://example.com/article")
...
def test_error_handling_consistency():
    ...
    with patch("duckduckgo_search.DDGS") as mock_ddgs:
        ...
        search_result = _web_search_impl(["test"])
    ...
    with patch("httpx.get") as mock_get:
        ...
        extract_result = _extract_content_impl("https://example.com")
